...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <form action="doAllDelete?flag=notice" method="post" >
        <h1 class="page-header">操作</h1>
        <ol class="breadcrumb">
          <li><a href="add-notice.jsp">增加公告</a></li>
        </ol>
        <h1 class="page-header">管理 <span id="noticeCount" class="badge"></span></h1>
...
goPage(1);
function goPage(cp){
	$.get('list',{'flag':'notice','cp':cp},function(data){
		var noticeInfo = '';
		for(var i = 0 ; i < data.list.length ; i++){
			noticeInfo += '<tr>';
			noticeInfo += '<td><input type="checkbox" class="input-control" name="checkbox[]" value="'+data.list[i].id+'" /></td>';
			var title = '';
			if(data.list[i].title.length > 25){
				title = data.list[i].title.substring(0,25)+'...';
			}else{
				title = data.list[i].title;
			}
			noticeInfo += '<td class="article-title">'+title+'</td>';
			...
			noticeInfo += '<td>'+time+'</td>';
			var id = data.list[i].id;
			noticeInfo += "<td><a href='listOne?flag=notice&id="+id+"'>修改</a> <a onclick='deleteUser("+id+")'>删除</a>";
			noticeInfo += '</tr>';
		}
		$('#noticeCount').html(data.list.length);
		$('#noticeInfo').html($(noticeInfo));
		genPageLink(data);
	})
}
...
function deleteUser(id){
	if (window.confirm("此操作会删除公告，是否确认？")) {
     $.ajax({
         type: "POST",
         url: "doDelete?flag=notice",
         data: "id=" + id,
         cache: false, //不缓存此页面   
...
function deleteChecked(){
	...
	if(arr.length>0){
		if(window.confirm('你已选了'+arr.length+'项,确认删除吗？')){
			$.get('doAllDelete?flag=notice&data='+arr,function(data){
				alert(data);
			});
		}
	}else{
