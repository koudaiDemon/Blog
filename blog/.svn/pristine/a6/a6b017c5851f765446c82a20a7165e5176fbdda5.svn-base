...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <form action="doDelete?flag=doVDelete" method="post" id="vdelete" onsubmit="return delVolum()">
        <h1 class="page-header">操作</h1>
        <ol class="breadcrumb">
          <li><a href="add-article.jsp">增加文章</a></li>
        </ol>
        <h1 class="page-header">管理 <span class="badge" id="articleCount">7</span></h1>
          ...
          <nav>
            <div class="btn-toolbar operation" role="toolbar">
              ...
              <div class="btn-group" role="group">
                <button type="submit" class="btn btn-default" data-toggle="tooltip" data-placement="bottom" title="删除全部选中" name="checkbox_delete" form="vdelete">删除</button>
              </div>
            </div>
...
function goPage(cp){
	//标题 栏目 标签 评论 日期 操作
 	$.get('list',{'flag':'article','cp':cp},function(data){
		var tbody = '';
		for(var i = 0 ; i < data.list.length ; i++){
				tbody += '<tr>';
				tbody += '<td>';
				tbody += '<input type="checkbox" class="input-control" name="checkbox[]" value="' + data.list[i].id + '">';
				tbody += '</td>';
				tbody += '<td>'+ data.list[i].title +'</td>';
				tbody += '<td>'+ data.list[i].category.name +'</td>';
				tbody += '<td>'+ data.list[i].category.keywords +'</td>';
				tbody += '<td>'+ data.list[i].feedbackNum +'</td>';
				var nowTime = new Date();
				nowTime.setTime(data.list[i].submitTime);
		 		var time = nowTime.getFullYear()+'年'+(nowTime.getMonth()+1)+'月'+nowTime.getDate()+'日  '+nowTime.getHours()+':'+nowTime.getMinutes()+':'+nowTime.getSeconds();
				tbody += '<td>'+ time +'</td>';
				tbody += '<td><a href="listOne?flag=article&id='+data.list[i].id+'">修改</a> <a rel="'+ data.list[i].id +'" onclick=checkDelete('+ data.list[i].id +')>删除</a></td>';
				tbody += '</tr>';
		}
		$('#articleCount').html(data.list.length)
		$('#decrition').html($(tbody));
		genPageLink(data);
	})	
}
...
function del(id) {
	if (window.confirm('是否确认删除？')) {
		location.href = 'doDelete?flag=article&id=' + id;
	}
}
</script>
</body>
