    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <div class="col-md-5">
          ...
          <form action="doAdd?flag=category" method="post" autocomplete="off">
        ...
        <div class="col-md-7">
          <h1 class="page-header">管理 <span class="badge" id="categoryCount"></span></h1>
	...
	$.get('list',{'flag':'category','cp':'1'},function(data){
		...
		for(var i = 0 ; i < data.list.length ; i++){
			if(data.list[i].pid == '0'){
				count++;
				tbody += '<tr>';
				tbody += '<td>'+data.list[i].id+'</td>';
				tbody += '<td>'+data.list[i].name+'</td>';
				tbody += '<td>'+data.list[i].vname+'</td>';
				tbody += '<td>'+data.list[i].acount+'</td>';
				tbody += '<td><a href="listOne?flag=categorytype&id='+data.list[i].id+'">修改</a> <a rel="'+data.list[i].id+'" onclick=checkDelete('+data.list[i].id+')>删除</a></td>';
				tbody += '</tr>';
			}
		}
		$('#categoryCount').html(count);
		$('#decrition').append($(tbody));
	})
	//加载选项
	$.get('list',{'flag':'category-type'},function(data){
		var options = '<option value="0" selected>无</option>';
		for(var i = 0 ; i < data.length ; i++){
			options += '<option value="'+data[i].id+'">'+data[i].name+'</option>';
		}
		$('#category-fname').append($(options));
	})
...
// 	alert(id);
	var msg = window.prompt('请输入:删除','');
	if(msg == '删除'){
		if(window.confirm("此操作不可逆，是否确认？"))
		{
			$.ajax({
				type: "POST",
				url: "doDelete?flag=category",
				data: "id=" + id,
				cache: false, //不缓存此页面   
