...
<title>友情链接 - SOFTEEM博客管理系统</title>
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <form action="/Flink/checkAll" method="post" >
        ...
        <ol class="breadcrumb">
          <li><a href="add-flink.jsp">增加友情链接</a></li>
        </ol>
        <h1 class="page-header">管理 <span class="badge" id="flinkCount"></span></h1>
...
<!--                 <td class="article-title">SOFTEEM</td> -->
<!--                 <td>http://www.ylsat.com</td> -->
<!--                 <td><a href="update-flink.jsp">修改</a> <a rel="6">删除</a></td> -->
<!--               </tr> -->
            </tbody>
          </table>
        </div>
...
$(function(){
	$.get('list',{'flag':'flink','cp':'1'},function(data){
// 		<tr>
//         	<td><input type="checkbox" class="input-control" name="checkbox[]" value="" /></td>
//         	<td class="article-title">SOFTEEM</td>
//         	<td>http://www.ylsat.com</td>
...
//       </tr>
		...
		for(var i = 0 ; i < data.list.length ; i++){
			tbody += '<tr>';
			tbody += '<td><input type="checkbox" class="input-control" name="checkbox[]" value="" /></td>';
			tbody += '<td class="article-title">'+data.list[i].name+'</td>';
			tbody += '<td>'+data.list[i].url+'</td>';
			tbody += '<td><a href="listOne?flag=flink&id='+data.list[i].id+'">修改</a> <a onclick=deletelink("'+data.list[i].id+'")>删除</a></td>';
			tbody += '</tr>';
		}
		$('#flinkCount').html(data.list.length);
		$('#showlinks').append($(tbody));
	})
...
function deletelink(id){
	console.info('别点我了');
	if (window.prompt('请输入:删除','') == "删除") 
	{
		if(window.confirm("此操作不可逆，是否确认？"))
		{
			$.ajax({
				type: "POST",
				url: "doDelete?flag=flink",
				data: "id=" + id,
				cache: false, //不缓存此页面   
...
function deleteChecked(){
	...
	if(arr.length>0){
		if(window.confirm('你已选了'+arr.length+'项,确认删除吗？')){
			$.get('doAllDelete?flag=flink&data='+arr,function(data){
				alert(data);
			});
		}
	}else{
