...
<%@page import="com.softeem.dto.Article"%>
...
<title>文章修改 - SOFTEEM博客管理系统</title>
...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <form action="doUpdate" method="post" id="updateArticleInfo" class="add-article-form">
        <input type="hidden" name="flag" value="article" />
          <div class="col-md-9">
...
<!--             </div> -->
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
              	<textarea class="form-control" id="describe" name="describe" autocomplete="off"></textarea>
...
$(function () {
    ...
    _uploadEditor.ready(function () {
  	...
  	//栏目栏动态显示
	$.get('list',{'flag':'category','cp':'1'},function(data){
		var count = 0;
		var ul = '';
		for(var i = 0 ; i < data.list.length ; i++){
			if(data.list[i].pid == '0'){
				count++;
				ul += '<li>';
				ul += '<input name="category" type="radio" value="'+data.list[i].id+'">';
				ul += '<label>'+data.list[i].name+'</label>';
				ul += '</li>';
			}
		}
		$('#ccc').html($(ul));
...
function reset(){
	var article = <%= JSON.toJSONString((Article)request.getAttribute("article"))%>;
	$('#id').val(article.id);
	$('#describe').val(article.description);
	
	var category = $('#ccc :radio');
	for(var i = 0 ; i < category.length ; i++){
		if(category.eq(i).attr('value') == article.category.id){
			category.eq(i).prop('checked','true');
		}
	}
	
	$('#imgSrc').val(article.imageSrc);
	var lastLoadTime = new Date();
	lastLoadTime.setTime(article.submitTime);
	...
	$('#submitTime').val(time);
	$('#headImg a').attr('href',base+article.imageSrc);
	$('#headImg img').attr('src',base+article.imageSrc);
...
function uploadImg(){
	...
	$('#headImgFile').change(function(){
		$.ajax(
				{ url: 'uploadImg?flag=article'
				, type: 'POST'
				, cache: false
				, data: new FormData($('#updateArticleInfo')[0])
