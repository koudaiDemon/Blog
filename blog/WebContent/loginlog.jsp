...
<title>登录记录 - SOFTEEM博客管理系统</title>
...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
        ...
        <ol class="breadcrumb">
          <li><a href="doAllDelete?flag=deleteAll">清除所有登录记录</a></li>
          <li><a href="doAllDelete?flag=currentLogin&mid=${manager.id}">清除本人登录记录</a></li>
        </ol>
...
goPageAllLoginInfo(1);
function goPageAllLoginInfo(cp){
	$.get('list?flag=loadInfoAll',{'cp':cp},function(data){
// 	console.info(data);
	var tr = ''
	for(var i = 0 ; i < data.list.length ; i++){
			...
			var nickName = '';
			if(data.list[i].manager != null){
				if(data.list[i].user != null){
					nickName = data.list[i].user.nickName;
				}else{
					nickName = data.list[i].manager.username;
				}
			}else{
				nickName = '游客';
			}
			tr += '<td class="article-title">'+nickName+'</td>';
			...
			tr += '<td><a onclick="deleteLoginLog('+data.list[i].id+')">删除</a></td>';
			tr += '</tr>';
		}
		$('#allLoadInfo').html($(tr));
		genPageLink(data);
...
function genPageLink(pd){
	...
	if(pd.currentPage == 1){
		ul += '<li class="disabled"><a href="javascript:goPageAllLoginInfo('+pd.pre+')">&laquo;</a></li>'
	}else{
		ul += '<li><a href="javascript:goPageAllLoginInfo('+pd.pre+')">&laquo;</a></li>'
	}
	for(var i = start ; i <= end ; i++){
		//如果当前遍历到的页码数等于当前页，则当前页码被激活class='active'
		if(pd.currentPage == i){
			ul += '<li class="active"><a href="javascript:goPageAllLoginInfo('+i+')">'+i+'</li>';
		}else{
			ul += '<li><a href="javascript:goPageAllLoginInfo('+i+')">'+i+'</li>';
		}
	}
	if(pd.currentPage == pd.totalPage){
		ul += '<li class="disabled"><a href="javascript:goPageAllLoginInfo('+pd.next+')">&raquo;</a></li>'
	}else{
		ul += '<li><a href="javascript:goPageAllLoginInfo('+pd.next+')">&raquo;</a></li>'
	}
	ul +='</ul>';
	$('#paging').html($(ul));
}
...
function deleteLoginLog(id){
	if (window.confirm("此操作会删除该条的记录，是否确认？")) {
     $.ajax({
         type: "POST",
         url: "doDelete?flag=loginLog",
         data: "id=" + id,
         cache: false, //不缓存此页面   
